...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['addUserForm'])) { 
    ...
    $class_id = null; // Initialize class_id
    ...
    try {
        ...
        $sql = "INSERT INTO users (username, password, role, full_name, email, status, class_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            throw new Exception("Prepare failed: (" . $conn->errno . ") " . $conn->error);
        }

        // Assign class_id only if role is student and class is selected
        if ($role === 'student' && isset($_POST['student_class']) && !empty($_POST['student_class'])) {
            $class_id = (int)$_POST['student_class']; // Cast to integer
        }

        // Bind parameters including the potentially null class_id
        $stmt->bind_param("ssssssi", $username, $password, $role, $full_name, $email, $status, $class_id);
        ...
        if (strpos($exceptionMessage, 'Duplicate entry') !== false) {
            if (strpos($exceptionMessage, "'username'") !== false) {
             ...
             $error_details = "An unexpected error occurred during user creation. Please check logs or contact support."; // More generic user message
         }
